gjrm <- function(formula, data = list(), weights = NULL, subset = NULL,  
  ...
  opc  <- c("N","C0","C90","C180","C270","J0","J90","J180","J270","G0","G90","G180","G270","F","AMH","FGM","T","PL","HO","GA")
  ...
  m3d  <- c("DEL","SICHEL")
  #MAP ALL COPULAS (AND ROTATIONS IF ADDED) TO "VineCopula" FAMILY (60+ IF NOT APPLICABLE)
  ...
  #CHECK formula IS LIST
  if(!is.list(formula)) stop("You must specify a list of equations.")
  #NUMBER OF FORMULAS (eq.mu.1, eq.mu.2, eq.sigma2.1, eq.sigma2.2, eq.theta)
  l.flist <- length(formula)
  #CHECK THAT ALL 5 EQUATIONS/FORMULAS ARE SUITABLE
  ...
  #v1 = "Price" "t"     "Oil"   "Coal" 
  v1     <- pred.varR$v1
  #v2 = "Demand" "t"      "Oil"    "Gas"    "Coal" 
  v2     <- pred.varR$v2
  #v2 = "Price"  "t"      "Oil"    "Coal"   "Demand" "Gas" 
  pred.n <- pred.varR$pred.n  
  #CREATES A FAKE FORMULA (Price ~ Price + t + Oil + Coal + Demand + Gas)
  fake.formula <- paste(v1[1], "~", paste(pred.n, collapse = " + "))
  #<environment: R_GlobalEnv>
  environment(fake.formula) <- environment(formula[[1]])
  #CREATE mf TO FORMAT DATA
  ...
  #FORMATS DATA (AND ONLY GETS REQUIRED COLUMNS)
  data <- eval(mf, parent.frame())
  #NOT APPLICABLE
  if(gc.l == TRUE) gc()  
  #NUMBER OF OBSERVATIONS (1784)
  n <- dim(data)[1]
  #ADD COLULM "(weights)" FILLED WITH 1S
  ...
  M$K1 <- NULL
  #CHECK ARGUMENTS ARE SUITABLE
  pream.wm(formula, margins, M, l.flist)
  #GET EQUATION 1 (eq.mu.1)
  formula.eq1 <- formula[[1]]
  #GET EQUATION 2 (eq.mu.2)
 ...
 #form.eq12R = (Price + mean(Price))/2 ~ s(t, k = 60) + s(Oil) + s(Coal)
 formula.eq1  <- form.eq12R$formula.eq1
 #formula.eq1r = Price ~ s(t, k = 60) + s(Oil) + s(Coal)
...
if(surv == TRUE && margins[1] %in% bl){ 
 ...
 #CHANGE FORMULA ATTRIBUTE OF gam1 TO formula.eq1r
 gam1$formula <- formula.eq1r
 #NUMBER OF SMOOTHS FORM gam1 (3, FOR t, Coal, Oil)
 lsgam1 <- length(gam1$smooth)
 #OVERWRITE y1 (BOTH SAME HERE ANYWAY)
 y1 <- y1.test
 #NOT APPLICABLE
 ...
 #APPLICABLE
 if( !(surv == TRUE && margins[1] %in% bl) ){
 #CHANGE (Price + mean(Price))/2 TO Price
 names(gam1$model)[1] <- as.character(formula.eq1r[2])
 #1784x78 PREDICTIONS FOR MODEL PARAMETERS (intercept, 59t, 9Oil, 9Coal)
 X1 <- predict(gam1, type = "lpmatrix")
 #HOW MANY sp IN gam1 (3, for s(t), s(Oil), s(Coal))
 l.sp1 <- length(gam1$sp)
 #sp VALUES (3, for s(t), s(Oil), s(Coal))
 sp1 <- gam1$sp
 }
 #NUMBER OF PARAMETRIC, NON-SMOOTH, MODEL TERMS INCLUDING THE INTERCEPT (1)
 gp1 <- gam1$nsdf 
 #NUMBER OF COLUMNS IN X1 (78)
 X1.d2 <- dim(X1)[2]
 ##############################################################
 # Equation 2 
 ##############################################################  
 ...
 #formula.eq2 = (Demand + mean(Demand))/2 ~ s(t, k = 60) + s(Oil) + s(Gas) + s(Coal)
 formula.eq2  <- form.eq12R$formula.eq1
 #formula.eq2r = Demand ~ s(t, k = 60) + s(Oil) + s(Gas) + s(Coal)
 formula.eq2r <- form.eq12R$formula.eq1r
 #1784 y VALUES (Demand)
 y2           <- form.eq12R$y1
 #SAME AS y1
 ...
 #CHANGE FORMULA ATTRIBUTE OF gam2 TO formula.eq2r
 gam2$formula <- formula.eq2r
 #NUMBER OF SMOOTHS FORM gam2 (4, FOR t, Coal, Oil, Gas)
 lsgam2 <- length(gam2$smooth)
 #OVERWRITE y2 (BOTH SAME HERE ANYWAY)
 y2 <- y2.test
 #NOT APPLICABLE
 ...
 #APPLICABLE
 if( !(surv == TRUE && margins[2] %in% bl) ){
 #CHANGE (Demand + mean(Demand))/2 TO Demand
 names(gam2$model)[1] <- as.character(formula.eq2r[2])
 #1784x87 predictions for model parameters (intercept, 59t, 9Oil, 9gas, 9Coal)
 X2 <- predict(gam2, type = "lpmatrix")
 #HOW MANY sp IN gam1 (4, for s(t), s(Oil), s(Gas), s(Coal))
 l.sp2 <- length(gam2$sp)
 #sp VALUES (4, for s(t), s(Oil), s(Gas), s(Coal))
 sp2 <- gam2$sp
 }
 #NUMBER OF PARAMETRIC, NON-SMOOTH, MODEL TERMS INCLUDING THE INTERCEPT (1)
 gp2 <- gam2$nsdf
 #NUMBER OF COLUMNS IN X1 (87)
 X2.d2 <- dim(X2)[2]
 #################################################################
 # Starting value for dependence parameter (and dof for T if used)
 #################################################################
 #1784 RESIDUALS FROM gam1
 res1 <- residuals(gam1)
 #1784 RESIDUALS FROM gam2
 res2 <- residuals(gam2)
 
#GET CORRELATION (KENDALL's TAU) OF res1 AND res2 VECTORS
ass.s <- cor(res1, res2, method = "kendall")
#TRUNCATE KENDALL's TAU TO -0.9<=X<=0.9
ass.s <- sign(ass.s)*ifelse(abs(ass.s) > 0.9, 0.9, abs(ass.s))
#COMPUTE THE PARAMATER OF BIVARIATE COPULA FOR A GIVEN KENDALL'S TAU, AND TRANSFORM USING TRANSFORMATION FUNCTION
...
#USED TO GET log.sig2.1
start.snR <- startsn(margins[1], y1)
#ESTIMATED log.sig2 PARAMETER USED TO CONSTRUCT A DENSITY FROM THE ASSUMED DISTRIBUTION; SET ITS NAME TO sigma1.star
...
#ESTIMATED log.sig2 PARAMETER USED TO CONSTRUCT A DENSITY FROM THE ASSUMED DISTRIBUTION; SET ITS NAME TO sigma2.star
...
#SET STARTING VALUES, BASED ON gam1, gam2, AND IF REQUIRED log.sig2 and i.rho
...
##############################################################  
    #APPLICABLE
    if(l.flist > 2){
    #MODEL OTHER MODEL PARAMETERS USING GAMS
    overall.svGR <- overall.svG(formula, data, ngc = 2, margins, M, vo, gam1, gam2, knots = knots)
    #SET STARTING VALUES, BASED ON gam1, gam2, AND IF REQUIRED USE GAMS FOR OTHER MODEL PARAMETERS; (int,59t,9oil,9coal, int,59t,9oil,9gas,9coal, int,59t, int,59t,9oil,9gas, int,59t)
...
#PROVIDES PENALTY MATRICES IN A FORMAT SUITABLE FOR AUTOMATIC MULTIPLE SMOOTHING PARAMETER ESTIMATION
qu.mag <- S.m(GAM, L.SP, L.GAM)                             
                                                        }
##########################################################
# general lists
##########################################################
#SET DEFAULT PARSCALE
...
#####my.env      <- new.env()
#SET k1 AND k2 (DECLARED IN gjrm CALL)
my.env$k1   <- k1.tvc
my.env$k2   <- k2.tvc
...
#APPLICABLE
if(gamlssfit == TRUE){ 
  #GET GAMLSS FORMULAS FOR EACH MARGINAL DISTRIBUTION
  form.gamlR <- form.gaml(formula, l.flist, M)
  #SET surv
  surv1 <- surv2 <- surv
  #NOT APPLICABLE
  if(surv == TRUE && margins[1] %in% c(m2,m3) && margins[2] %in% bl ) surv1 <- FALSE 
  #FIT UNIVARIATE REGRESSION FOR MODEL PARAMETERS OF FIRST MARGIN
  ...
  #FIT UNIVARIATE REGRESSION FOR MODEL PARAMETERS OF SECOND MARGIN
  ...
  #GET STARTING VALUES AND SPS FROM GAMLSS FITS
  ...
  #COPY Y VALUES TO y1.m and y2.m; NOT APPLICABLE
  y1.m <- y1; if(margins[1] == "LN") y1.m <- exp(y1) 
  y2.m <- y2; if(margins[2] == "LN") y2.m <- exp(y2)
  #OVERWRITE SemiParFit WITH SemiParFit FROM SemiParFit.p
  SemiParFit <- SemiParFit.p$SemiParFit  
  #NOT APPLICABLE
  if(gc.l == TRUE) gc()
  ##########################################################################################################################
#CHECK MODEL IS SUITABLE
cov.c(SemiParFit)
##########################################################################################################################
#####INSERT DATA TO ALL GAMS
